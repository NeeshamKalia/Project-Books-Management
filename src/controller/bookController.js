...
const createBook = async function (req, res) {
  try {
    let data = req.body;
    /*----------------------------VALIDATION STArTS------------------------*/
    if (!isValidBody(data))return res.status(400).send({ status: false, message: "Please provide Details in body" });
    ...
    if (checkTitle)return res.status(400).send({ status: false, message: `title =>${title} is already Taken` });
...
//-----------------------------------------------------------------------------//
    ...
    if(search.length==0)return res.status(404).send({ Status: false, message: "The Books you are searching for might be deleted by you before or not created yet" })
...
const deleteBookbyId = async function (req, res) {
  try {
    ...
    if (!searchBook)return res.status(404).send({ status: false, message: "Not Found.." });
